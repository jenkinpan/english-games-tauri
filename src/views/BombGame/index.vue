    ...
    <div class="bomb-game-container">
        ...
        <div class="container">
            ...
            <div class="score-container">
                <button class="btn" @click="startGame" :disabled="(gameStarted && !gameOver) || isAnimatingBomb">
                    开始游戏
                </button>
                <button class="btn reset" @click="resetGame" :disabled="isAnimatingBomb">
                    重置
                </button>
                <div style="display: flex; align-items: center; gap: 8px">
                    <label for="bombCountInput" style="font-weight: bold; color: #2c3e50">炸弹数量</label>
                    <input id="bombCountInput" type="number" min="1" :max="Math.max(1, words.length - 1)"
                        v-model.number="bombCount" @change="updateBombCountConstraints" :disabled="isAnimatingBomb"
                        style="
                            width: 80px;
                            padding: 8px;
                ...
                <div v-for="(card, index) in cards" :key="index" class="card" :class="{
                    ...
                    <div class="card-inner">
                        ...
                        <div class="card-back" :class="card.type" ref="cardBackRefs">
                            <div v-if="card.flipped && card.type === 'bomb'" class="bomb-icon">
                                💣
                            </div>
                            <div v-if="card.flipped && card.type === 'score'" class="score-value">
                                +{{ card.value }}
                            </div>
                        </div>
                    </div>
            ...
            <div class="input-section" :class="{ hidden: isInputHidden }">
                ...
                <div class="group-tabs-container">
                    <div class="tabs-scroll-area">
                        ...
                        <!-- 添加新分组按钮 -->
                        <div class="tab-add-btn" @click="openSaveGroupModal(null)">
                            +
                        </div>
                    </div>
                </div>
                ...
                <div style="
                    ...
                    <div style="display: flex; gap: 8px; flex-wrap: wrap;">
                        ...
                        <!-- 单词操作按钮 -->
                        <button class="btn" @click="addWord" :disabled="isAnimatingBomb" style="
                                padding: 8px 16px;
                                font-size: 14px;
                        ...
                        <button class="btn" @click="removeWord" :disabled="isAnimatingBomb" style="
                                padding: 8px 16px;
                                font-size: 14px;
                        ...
                        <button class="btn" @click="requestClearWords" :disabled="isAnimatingBomb" style="
                                padding: 8px 16px;
                                font-size: 14px;
                ...
                <div class="word-inputs">
                    <div v-for="(_word, index) in words" :key="index" class="input-group">
                        ...
                        <input type="text" v-model="words[index]" :placeholder="`输入单词 ${index + 1}`"
                            @input="handleWordInput(index)" autocapitalize="off" autocorrect="off" spellcheck="false" />
                    </div>
                </div>
            </div>
            ...
            <div v-if="showGroupModal" class="modal-overlay">
                <div class="modal-content">
                    ...
                    <p>请输入分组名称：</p>
                    <input type="text" v-model="groupNameInput" placeholder="例如：剑桥一级" style="
                            width: 100%;
                            padding: 10px;
                            margin: 10px 0;
